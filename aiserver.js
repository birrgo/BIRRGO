...
const app = Express();
const PORT = process.env.AI_PORT || 3001; // Separate port from main server

// Set Admin Authorization Secret Token (Default: '808080')
const ADMIN_SECRET_TOKEN = process.env.ADMIN_SECRET_TOKEN || '808080';

// Enable CORS securely for birrgo.online and all origins
app.use(cors({
  origin: '*', 
...
// Initialize Firebase Admin securely using Render Environment Variables
if (process.env.FIREBASE_SERVICE_ACCOUNT) {
  try {
    const serviceAccount = JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT);
    // Initialize if it hasn't been initialized already (in case both files are run in same context)
    ...
    console.log("Firebase Admin securely connected on AI Server!");
  } catch (error) {
...
app.get('/', (req, res) => {
  res.send('BirrGo AI Backend is live and running!');
});

// ==========================================
// GROQ AI CHAT & MANAGEMENT ENDPOINTS
// ==========================================
